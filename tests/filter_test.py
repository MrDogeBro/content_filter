...
class TestFilter(unittest.TestCase):
    def accuracy_test(self):
        ...
        self.assertEqual(
            filter.check("Hi there, how are you doing today?").as_bool, False
        )
        self.assertEqual(filter.check("Im good, hbu?").as_bool, False)
        self.assertEqual(filter.check("I'm good, how about you?").as_bool, False)
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("The ants enjoyed the barbecue more than the family.").as_bool,
            False,
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("He found the chocolate covered roaches quite tasty.").as_bool,
            False,
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("Chocolate covered crickets were his favorite snack.").as_bool,
            False,
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(filter.check("I like to suck d***").as_bool, True)
        self.assertEqual(filter.check("what os this $h!+").as_bool, True)
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("I checked to make sure that he was still alive.").as_bool,
            False,
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(filter.check("u got a nice pu$$y girl?").as_bool, True)
        self.assertEqual(filter.check("go to he!l you idiot").as_bool, True)
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("She had a habit of taking showers in lemonade.").as_bool,
            False,
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("He waited for the stop sign to turn to a go sign.").as_bool,
            False,
        )
        self.assertEqual(filter.check("oh girl giv me dem t1+s").as_bool, True)
        self.assertEqual(
            filter.check("How about you shows me your b00b$ tonight?").as_bool, True
        )
        self.assertEqual(filter.check("Lets have $3x tonight baby girl?").as_bool, True)
        self.assertEqual(filter.check("Im gonna rap3 you little girl").as_bool, True)
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(filter.check("We have a lot of rain in June.").as_bool, False)
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("At that moment she realized she had a sixth sense.").as_bool,
            False,
        )
        self.assertEqual(
            filter.check("You would be a great p0rn star girl!").as_bool, True
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("Let me insert myself into that vjayjay girl").as_bool, True
        )
        self.assertEqual(
            filter.check("Don't be scared to drop dem panties girl").as_bool, True
        )
        self.assertEqual(
            filter.check("Make sure to lube up your vaj before we get down").as_bool,
            True,
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("Of course, she loves her pink bunny slippers.").as_bool, False
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(filter.check("Girl, r u a h0ok3r?").as_bool, True)
        self.assertEqual(filter.check("lets get naked up in here").as_bool, True)
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(filter.check("Hit me with your pet shark!").as_bool, False)
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("You always look best when your nude!").as_bool, True
        )
        self.assertEqual(
            filter.check("Be careful with that butter knife.").as_bool, False
        )
        self.assertEqual(
            filter.check("I am my aunt's sister's daughter.").as_bool, False
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check(
                "FFFFFFFFFFUUUUUUUUUUUUUUUUUUUUUUCCCCCCCCCCKKKKKKKKKKK ME!!!!!"
            ).as_bool,
            True,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            filter.check("You bite up because of your lower jaw.").as_bool, False
        )
        self.assertEqual(
            filter.check("The mysterious diary records the voice.").as_bool, False
        )
        self.assertEqual(
            filter.check("Hello there, how are you doing today?").as_bool, False
        )
    ...
    def filter_list_test(self):
        ...
        self.assertEqual(
            filter.check("You always look best when your nude!").as_list,
            [
                {
                    "find": "nude",
                    "word": "nude",
        ...
        self.assertEqual(
            filter.check("Hello there, how are you doing today?").as_list, []
        )
        self.assertEqual(
            filter.check("you wh0@r!").as_list,
            [
                {
                    "find": "hoar",
                    "word": "hoar",
        ...
        self.assertEqual(
            filter.check("You would be a great p0rn star girl!").as_list,
            [
                {
                    "find": "porn",
                    "word": "porn",
        ...
        self.assertEqual(
            filter.check("Don't be scared to drop dem panties girl").as_list,
            [
                {
                    "find": "pantie",
                    "word": "pantie",
    ...
    def filter_additions_test(self):
        ...
        self.assertEqual(filter.check("AREA 51 STATUS!!!!!!!").as_bool, False)

        filter.add_words(["rave", "raid", "area51", "69"])
        self.assertEqual(filter.check("Pat ordered a ghost pepper pie.").as_bool, False)
        self.assertEqual(filter.check("Lets go the the raveEEEEEE!!!!").as_bool, True)
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(
            filter.check("U wanna raid the new popular game?").as_bool, True
        )
        self.assertEqual(filter.check("6666666666999999999999").as_bool, True)
        self.assertEqual(
            filter.check("They were excited to see their first sloth.").as_bool, False
        )
        self.assertEqual(
            filter.check(
        ...
        self.assertEqual(filter.check("AREA 51 STATUS!!!!!!!").as_bool, True)
        self.assertEqual(
            filter.check("It was a really good Monday for being a Saturday.").as_bool,
            False,
        )
        ...
        self.assertEqual(filter.check("u headed to the rave tonight?").as_bool, True)
        self.assertEqual(
            filter.check(
    ...
    def filter_exceptions_test(self):
        ...
        self.assertEqual(
            filter.check("why the fuck is there nobody here").as_bool, False
        )
        self.assertEqual(filter.check("bitch what u lookin at").as_bool, True)
        self.assertEqual(filter.check("suck my dick").as_bool, True)
        self.assertEqual(
            filter.check("Hi there, what have you been doing?").as_bool, False
        )
        self.assertEqual(
            filter.check("Im doing fine myself, just working.").as_bool, False
        )
        self.assertEqual(filter.check("this SHHHI++++ is crazy!!!!").as_bool, True)
        self.assertEqual(filter.check("why are we even still friends?").as_bool, False)
        self.assertEqual(filter.check("u an asshole mate").as_bool, True)

        filter.add_exceptions(["bitch", "asshole"])
        self.assertEqual(filter.check("bitch what u lookin at").as_bool, False)
        self.assertEqual(filter.check("bitch look at yourself").as_bool, False)
        self.assertEqual(filter.check("Eat my shi+!").as_bool, True)
        self.assertEqual(filter.check("u an asshole mate").as_bool, False)
        self.assertEqual(
            filter.check("what the Fuck you think your up to?").as_bool, False
        )
        self.assertEqual(
            filter.check("Why is there so much hatred in the world nowadays?").as_bool,
            False,
        )
        self.assertEqual(filter.check("Where is everyone?").as_bool, False)
        self.assertEqual(
            filter.check("I turned 18 today!! Finally an adult!").as_bool, False
        )
